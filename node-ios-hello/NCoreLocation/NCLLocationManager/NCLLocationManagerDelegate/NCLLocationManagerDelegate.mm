...
//    mgr->_onAuthorization.Reset(Local<Function>::Cast(info[0]));
    mgr->_didUpdateLocations.Reset(Local<Function>::Cast(info[0]));
  
    @autoreleasepool {
         mgr->SetNSObject([[SCLLocationManagerDelegate alloc] initWithDidUpdateLocations: ^ (CLLocationManager* manager, NSArray<CLLocation *> * _Nonnull locations) {
          Nan::HandleScope scope;
