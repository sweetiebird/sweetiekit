...
//namespace {
    ...
    private:
        void MaybeWriteStartupBlob(const i::Vector<const i::byte>& blob) const {
            ...
            if (fp != nullptr && fp != stdout && fp != stderr && fp != stdin)
                fclose(fp);
        ...
        static FILE* GetFileDescriptorOrDie(const char* filename) {
            ...
            if (filename == nullptr) {
                ...
                size_t n = strlen(base) + strlen(filename) + 4096;
                char* path = (char*)malloc(n);
                memset(path, 0, n);
                snprintf(path, n, "%s/Documents/%s", base, filename);
                fp = v8::base::OS::FOpen(path, "wb");
            }
#else
            if (filename != nullptr) {
                fp = v8::base::OS::FOpen(filename, "wb");
            }
#endif
            if (fp == nullptr) {
...
char* iosFilePath(const char* filepath) {
    if (filepath != nullptr) {
        while (filepath && filepath[0] == '/')
            filepath = filepath + 1;
        char* home = getenv("HOME");
        size_t n = strlen(home) + 1 + strlen(filepath) + 1 + 4096;
        char* path = (char*)malloc(n);
        memset(path, 0, n);
        snprintf(path, n, "%s/Documents/%s", home, filepath);
        return path;
        //(lldb) p typedef char* (*getenv_t)(const char*); ((getenv_t)getenv)("HOME");
        //(char *) $5 = 0x00000002838c0665 "/private/var/mobile/Containers/Data/Application/9429A537-BB53-4DF4-87D4-0E6555BD2134"
    }
    return nullptr;
}

void mksnapshot_b(i::EmbeddedFileWriter& embedded_writer)
{
    fprintf(stderr, "after SnapshotCreator\n"); fflush(stderr);
    //if (i::FLAG_embedded_builtins)
    if (1)
    {
        fprintf(stderr, "before WriteEmbeddedFile\n"); fflush(stderr);
        //WriteEmbeddedFile(&embedded_writer);
        i::EmbeddedData embedded_blob = i::EmbeddedData::FromBlob();
        embedded_writer.WriteEmbedded(&embedded_blob);
        fprintf(stderr, "after WriteEmbeddedFile\n"); fflush(stderr);
    }
}
...
int mksnapshot_main(int argc, char** argv) {
    ...
    
    {
        ...
        snapshot_writer.SetSnapshotFile(iosFilePath(i::FLAG_startup_src));
        snapshot_writer.SetStartupBlobFile(iosFilePath(i::FLAG_startup_blob));
        
        i::EmbeddedFileWriter embedded_writer;
        embedded_writer.SetEmbeddedFile(iosFilePath(i::FLAG_embedded_src));
        ...
        v8::StartupData blob;
        {
            ...
            mksnapshot_b(embedded_writer);
            fprintf(stderr, "before CreateSnapshotDataBlob\n"); fflush(stderr);
            blob = CreateSnapshotDataBlob(&snapshot_creator, embed_script.get());
            fprintf(stderr, "after CreateSnapshotDataBlob\n"); fflush(stderr);
        }
...
extern "C" void run_mksnapshot() {
    char* argv[] = {(char*)"node"};
    mksnapshot_main(1, argv);
}

extern "C" void run_mksnapshot_argv(int argc, const char** argv)
{
    size_t n = 0;
    int i;
    ...
    char* args = (char*)malloc(n + 4096);
    memset(args, 0, n + 4096);
    char* dst = args;
    std::vector<char*> arg;
    for (i = 0; i < argc && argv[i] != nullptr; i++) {
        strcpy(dst, argv[i]);
...
extern "C" void run_mksnapshot_with_args(const char* args)
{
    std::vector<char*> arg;
    while (*args != '\0') {
        arg.push_back((char*)args);
        args += strlen(args) + 1;
        if (arg.size() > 100)
            __builtin_trap();
    }
