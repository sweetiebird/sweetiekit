...
id GetValueFor(Local<Value> value, bool* failed = nullptr) {
...
template<typename T>
Local<Value> js_value_id(T* value) {
  return sweetiekit::GetWrapperFor(value);
}

template<typename T>
T* to_value_id(Local<Value> value, bool* failed = nullptr) {
  return (T*)GetValueFor(value, failed);
}
...
NSArray<T>* _Nullable to_value_NSArray(Local<Value> arr) {
  if (!arr->IsArray()) {
    ...
    for (uint32_t i = 0, n = value->Length(); i < n; i++) {
      id item = GetValueFor(value->Get(i));
...
#define to_value_wrapper(x, t) (t*)GetValueFor(x)
