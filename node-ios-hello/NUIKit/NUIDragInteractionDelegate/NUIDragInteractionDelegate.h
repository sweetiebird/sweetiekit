//
//  NUIDragInteractionDelegate.h
//
//  Created by Shawn Presser on 7/4/2019.
//  Copyright © 2019 sweetiebird. All rights reserved.
//

#ifndef NUIDragInteractionDelegate_h
#define NUIDragInteractionDelegate_h    

#include "NNSObject.h"

#define js_value_UIDragInteractionDelegate(x) js_protocol_wrapper(x, UIDragInteractionDelegate)
#define to_value_UIDragInteractionDelegate(x) to_protocol_wrapper(x, UIDragInteractionDelegate)
#define is_value_UIDragInteractionDelegate(x) is_protocol_wrapper(x, UIDragInteractionDelegate)

JS_WRAP_PROTOCOL(UIDragInteractionDelegate, NSObject);
  JS_PROP(dragInteractionItemsForBeginningSession);
  JS_PROP(dragInteractionPreviewForLiftingItemSession);
  JS_PROP(dragInteractionWillAnimateLiftWithAnimatorSession);
  JS_PROP(dragInteractionSessionWillBegin);
  JS_PROP(dragInteractionSessionAllowsMoveOperation);
  JS_PROP(dragInteractionSessionIsRestrictedToDraggingApplication);
  JS_PROP(dragInteractionPrefersFullSizePreviewsForSession);
  JS_PROP(dragInteractionSessionDidMove);
  JS_PROP(dragInteractionSessionWillEndWithOperation);
  JS_PROP(dragInteractionSessionDidEndWithOperation);
  JS_PROP(dragInteractionSessionDidTransferItems);
  JS_PROP(dragInteractionItemsForAddingToSessionWithTouchAtPoint);
  JS_PROP(dragInteractionSessionForAddingItemsWithTouchAtPoint);
  JS_PROP(dragInteractionSessionWillAddItemsForInteraction);
  JS_PROP(dragInteractionPreviewForCancellingItemWithDefault);
  JS_PROP(dragInteractionItemWillAnimateCancelWithAnimator);
JS_WRAP_PROTOCOL_END(UIDragInteractionDelegate, NSObject);

#if __OBJC__
@interface UIDragInteractionDelegateType : NSObject<UIDragInteractionDelegate>
...
#endif /* NUIDragInteractionDelegate_h */
