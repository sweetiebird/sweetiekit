...
DrawableView = class DrawableView {
  constructor(...args) {
    this.api = UIImageView(...args);
  }

  begin() {
    this.end();
    this.ctx = UIGraphicsBeginImageContextWithOptions(this.api.size, false, UIScreen.mainScreen.scale);
  }

  end() {
    if (this.ctx) {
      this.image = UIGraphicsGetImageFromCurrentImageContext();
...
UIViewDraw_make = async function UIViewDraw_make(nav, demoVC) {
  ...
  ggDraw = new DrawableView();
  ggView = ggDraw.api;
  //ggView = global.ggView = UIVisualEffectView.alloc().initWithEffect(blurEffect);


  //CGContextSetStrokeColorWithColor(ctx, UIColor.red)
